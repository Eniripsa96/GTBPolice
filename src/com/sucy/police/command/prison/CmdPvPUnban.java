 ...
 */
public class CmdPvPUnban implements ICommand {
    ...
    public void execute(CommandHandler handler, Plugin plugin, CommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            ...
            // Not banned
                ...
                sender.sendMessage(ChatColor.DARK_RED + "That player is not banned from PvP");
            }

            // Ban them
            else {
                police.pvpUnban(args[0]);
                sender.sendMessage(ChatColor.DARK_GREEN + "You have unbanned them from PvP");
                if (player.isOnline()) {
                    police.getServer().getPlayer(player.getName()).sendMessage(ChatColor.DARK_RED + "You are no longer banned from PvP");
                }
            }
        }
